...
export class Level1 extends Level {
  ...
  updateRovers(time: Ticker): void {
    ...
    console.log(player.actionTimer)
    ...
    this.rovers.forEach(rover => {
      ...
      dangers.forEach(danger => {
        ...
        if (distance < minDangerDistance) {
          ...
          if (visible) {
            ...
            if (movingToDanger) rover.targetVelocity = mul(-2 * targetAgentSpeed, getRandomDir())
          }
        }
      })
      if (minDangerDistance < safeDistance) return
      if (player.actionTimer > 10) return
      ...
      if (distance < 400) {
        ...
        if (visible) {
          ...
          if (distance < 15) {
            if (movingToPlayer) rover.targetVelocity = mul(targetAgentSpeed, getRandomDir())
            return
          }
          if (distance > 30) {
            if (!movingToPlayer) rover.targetVelocity = mul(targetAgentSpeed, getRandomDir())
        ...
        if (!visible) {
          for (const point of player.history) {
            ...
            if (visible) {
              ...
              rover.targetVelocity = mul(targetAgentSpeed, targetDir)
              return
            }
          }
        }
      ...
      if (2000 * Math.random() < time.deltaMS) {
        rover.targetVelocity = mul(targetAgentSpeed, getRandomDir())
      }
    })
  }
